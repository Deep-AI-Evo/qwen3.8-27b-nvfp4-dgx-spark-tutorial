...
| 245K 顶格 prefill | 705 tok/s（5.8 min 灌入） | 与 vLLM 256K 的 715 几乎重合 |
...
### 4.2 长上下文（256K 全档实测，随机前缀无前缀缓存，散文内容）
...
| 150K | 150,022 | 157 s | 953 tok/s | 15.6 tok/s |
| 200K | 200,022 | 249 s | 805 tok/s | 13.1 tok/s |
| **245K（≈顶格）** | 245,022 | **347 s（5.8 min）** | 705 tok/s（-71%） | **14.0 tok/s（仅 -30%）** |

三个关键结论：

1. **245K 顶格可用**：模型原生 262,144 上下文直接生效（启动无需指定 context-length），
   一次灌入约 5.8 分钟，decode 仍有 14 tok/s。
2. **与 vLLM 同档几乎重合**（100K prefill 1,172 vs 1,230；顶格 705@245K vs 715@261K）——
   散文内容下 DSPARK 无额外加速，两引擎裸速趋同；SGLang 的速度优势在代码/英文短上下文。
3. **decode 只衰减 30%**（1K 的 20 → 245K 的 14），DSPARK 长上下文不坍塌。

**缓存命中的重复前缀 prefill 达 18K–71K tok/s**（RadixAttention 生效）——
这是 SGLang 路径相对 vLLM 的实际体验优势：长文档多轮问答，后续轮次 prefill 几乎免费。
